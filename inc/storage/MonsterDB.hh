...
struct MonsterPersistentStorage
{
    static Optional<Monster> get(int id)
    {
        ...
        get_mysql_connection()->session << "select * from monster where ID=:ID", soci::into(m), soci::use(id);
    ...
    {
        get_mysql_connection()->session << "delete from monster where id=:ID", soci::use(id);
    }

    static int insert(const Monster& m)
    {
        get_mysql_connection()->session << "insert into monster("
                ...
                soci::use(m);
        int id = 0;
        get_mysql_connection()->session << "select @@IDENTITY", soci::into(id);
        return id;
    }
    ...
    {
        get_mysql_connection()->session << "update monster set " + filed + "=:VALUE where id=:ID", soci::use(value), soci::use(id);
    }
};
