...
struct MonsterPersistentStorage
{
    static Optional<Monster> get(int id)
    {
        ...
        DBAccess::mysql_query([&m, id](soci::session& session)
        {
            session << "select * from monster where ID=:ID", soci::into(m), soci::use(id);
        });
    ...
    {
        DBAccess::mysql_query([id](soci::session& session)
        {
            session << "delete from monster where id=:ID", soci::use(id);
        });
    }
        ...
        DBAccess::mysql_query([&m, &id](soci::session& session)
        {
            session << "insert into monster("
                ...
                ") values("
                    ...
                    ":sdef_lv, :spd_lv, :acc_lv, :crit_lv, :skill1, :skill2, :skill3, :skill4, :ability)", 
                soci::use(m);
            session << "select @@IDENTITY", soci::into(id);
        });
        return id;
    }
    ...
    {
        DBAccess::mysql_query([&filed, &id, &value](soci::session& session)
        {
            session << "update monster set " + filed + "=:VALUE where id=:ID", soci::use(value), soci::use(id);
        });
    }
};
